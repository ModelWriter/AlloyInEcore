...
public class AIESyntacticReconcilingStrategy
  ...
  private List<Annotation> newAnnotations = new ArrayList<>();
  ...
  public void reconcile(final IRegion partition) {
    try {
      ...
      iFile.deleteMarkers(AIEEditor.UNSAT_ERROR_MARKER, true, IResource.DEPTH_INFINITE);
  ...
  protected void removeOldAnnotations() {
    ...
    Iterator<Annotation> annotatitonIterator = annotationModel.getAnnotationIterator();
    while (annotatitonIterator.hasNext()) {
      Annotation annotation = annotatitonIterator.next();
      if (annotation.getType().equals(AIEEditor.UNSAT_ERROR_ANNOTATION)) {
        annotationModel.removeAnnotation(annotation);
      }
    }

    oldAnnotations.forEach(anno -> annotationModel.removeAnnotation(anno));
