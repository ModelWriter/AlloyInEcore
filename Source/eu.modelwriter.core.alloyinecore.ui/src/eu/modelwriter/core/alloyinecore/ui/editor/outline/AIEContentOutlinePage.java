      ...
      public void selectionChanged(final SelectionChangedEvent event) {
        if (!event.getSelectionProvider().getSelection().equals(selection)) {
          ...
          if (selection instanceof IStructuredSelection) {
            final java.lang.Object item = ((IStructuredSelection) selection).getFirstElement();
            if (item instanceof Element && !((Element<?>) item).isImported()) {
      ...
      public void doubleClick(DoubleClickEvent event) {
        ...
        if (selection instanceof IStructuredSelection) {
          ...
          if (item instanceof Element) {
            Element<?> selectedElement = (Element<?>) item;
            AIEHyperlinkDetector.createHyperlink(selectedElement, aieEditor.getIFile()).open();
          }
        }
      }
    });
