...
import eu.modelwriter.core.alloyinecore.recognizer.AlloyInEcoreParser.ReasonContext;
...
public class CS2ASMapping extends AlloyInEcoreBaseVisitor<Object> {
  ...
  public EObject parseAndSave(final String fileInput, final URI saveURI) {
    ...
    final ParseTree tree = parser.model(null);
    ...
    try {
      ...
      final String string = serializeModel(repository.getRootPackage(), saveURI);
      ...
      final String string = serializeModel(oldRoot, saveURI);
      final EAnnotation sourceAnnotation = createEAnnotation(AnnotationSources.SOURCE);
      sourceAnnotation.getDetails().put(AIEConstants.SOURCE.toString(), fileInput);
      sourceAnnotation.getDetails().put(AIEConstants.SOURCE_HASH.toString(),
          string.hashCode() + "");
  ...
  public String serializeModel(final EModelElement model, final URI saveURI) {
  ...
  public EClass visitEClass(final EClassContext ctx) {
    ...
    if (ctx.cardinality != null) {
      final EAnnotation multiplicityAnnotation = createEAnnotation(AnnotationSources.CARDINALITY);
      multiplicityAnnotation.getDetails().put(AIEConstants.CARDINALITY.toString(),
          ctx.cardinality.getText());
      eClass.getEAnnotations().add(multiplicityAnnotation);
    }

    if (ctx.upperScope != null) {
      final EAnnotation scopeAnnotation = createEAnnotation(AnnotationSources.UPPERSCOPE);
      scopeAnnotation.getDetails().put(AIEConstants.UPPERSCOPE.toString(),
          ctx.upperScope.getText());
      eClass.getEAnnotations().add(scopeAnnotation);
    }

    if (ctx.lowerScope != null) {
      final EAnnotation scopeAnnotation = createEAnnotation(AnnotationSources.LOWERSCOPE);
      scopeAnnotation.getDetails().put(AIEConstants.LOWERSCOPE.toString(),
          ctx.lowerScope.getText());
      eClass.getEAnnotations().add(scopeAnnotation);
    }

    if (ctx.lowerIncluded != null) {
      final EAnnotation scopeAnnotation = createEAnnotation(AnnotationSources.LOWERINCLUDED);
      scopeAnnotation.getDetails().put(AIEConstants.LOWERINCLUDED.toString(),
          ctx.lowerIncluded.getText());
      eClass.getEAnnotations().add(scopeAnnotation);
    }

    if (ctx.upperIncluded != null) {
      final EAnnotation scopeAnnotation = createEAnnotation(AnnotationSources.UPPERINCLUDED);
      scopeAnnotation.getDetails().put(AIEConstants.UPPERINCLUDED.toString(),
          ctx.upperIncluded.getText());
      eClass.getEAnnotations().add(scopeAnnotation);
    }
  ...
  public EAttribute visitEAttribute(final EAttributeContext ctx) {
    ...
    // if the EAttribute is a model feature, do not create!
    List<String> qualifiers = null;
    EAnnotation modelFeatureAnnotation = null;
    EModelElement owner = null;
    if (ctx.qualifier != null) {
      qualifiers =
          ctx.qualifier.stream().map(Token::getText).distinct().collect(Collectors.toList());
      if (qualifiers.contains(AIEConstants.MODEL.toString())) {
        modelFeatureAnnotation = createEAnnotation(AnnotationSources.MODEL);

        String modelFieldText = "";
        if (ctx.start != null && ctx.stop != null) {
          modelFieldText = getTokenString(ctx.start.getStartIndex(), ctx.stop.getStopIndex() + 1);
        }

        modelFeatureAnnotation.getDetails().put(AIEConstants.MODEL.toString(), modelFieldText);
        owner = (EModelElement) repository.getEObject(qualifiedNameStack);
        owner.getEAnnotations().add(modelFeatureAnnotation);
      }
    }
    ...
    if (ctx.cardinality != null) {
      ...
      multiplicityAnnotation.getDetails().put(AIEConstants.CARDINALITY.toString(),
          ctx.cardinality.getText());
      eAttribute.getEAnnotations().add(multiplicityAnnotation);
    }
    ...
    if (modelFeatureAnnotation != null) {
      modelFeatureAnnotation.getContents().add(eAttribute);
      ((EClass) owner).getEStructuralFeatures().remove(eAttribute);
      return null;
    } else
      return eAttribute;
  }
  ...
  public EReference visitEReference(final EReferenceContext ctx) {
    ...
    // if the EReference is a model feature, do not create!
    List<String> qualifiers = null;
    EAnnotation modelFeatureAnnotation = null;
    EModelElement owner = null;
    if (ctx.qualifier != null) {
    ...
    if (ctx.cardinality != null) {
      ...
      multiplicityAnnotation.getDetails().put(AIEConstants.CARDINALITY.toString(),
          ctx.cardinality.getText());
      eReference.getEAnnotations().add(multiplicityAnnotation);
    }
    ...
    if (ctx.reason != null) {
      final EAnnotation eAnnotation = visitReason(ctx.reason);
      ...
      for (final String q : qualifiers) {
        switch (AIEConstants.getValue(q)) {
          ...
          case GHOST:
            ...
            break;
          case ACYCLIC:
            final EAnnotation acyclicAnnotation = createEAnnotation(AnnotationSources.ACYCLIC);
            // DEFAULT NULL
            eReference.getEAnnotations().add(acyclicAnnotation);
            break;
          case TRANSITIVE:
            final EAnnotation transitiveAnnotation =
                createEAnnotation(AnnotationSources.TRANSITIVE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(transitiveAnnotation);
            break;
          case REFLEXIVE:
            final EAnnotation reflexiveAnnotation = createEAnnotation(AnnotationSources.REFLEXIVE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(reflexiveAnnotation);
            break;
          case IRREFLEXIVE:
            final EAnnotation irreflexiveAnnotation =
                createEAnnotation(AnnotationSources.IRREFLEXIVE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(irreflexiveAnnotation);
            break;
          case SYMMETRIC:
            final EAnnotation symmetricAnnotation = createEAnnotation(AnnotationSources.SYMMETRIC);
            // DEFAULT NULL
            eReference.getEAnnotations().add(symmetricAnnotation);
            break;
          case ASYMMETRIC:
            final EAnnotation asymmetricAnnotation =
                createEAnnotation(AnnotationSources.ASYMMETRIC);
            // DEFAULT NULL
            eReference.getEAnnotations().add(asymmetricAnnotation);
            break;
          case ANTISYMMETRIC:
            final EAnnotation antisymmetricAnnotation =
                createEAnnotation(AnnotationSources.ANTISYMMETRIC);
            // DEFAULT NULL
            eReference.getEAnnotations().add(antisymmetricAnnotation);
            break;
          case TOTAL:
            final EAnnotation totalAnnotation = createEAnnotation(AnnotationSources.TOTAL);
            // DEFAULT NULL
            eReference.getEAnnotations().add(totalAnnotation);
            break;
          case FUNCTIONAL:
            final EAnnotation functionalAnnotation =
                createEAnnotation(AnnotationSources.FUNCTIONAL);
            // DEFAULT NULL
            eReference.getEAnnotations().add(functionalAnnotation);
            break;
          case SURJECTIVE:
            final EAnnotation surjectiveAnnotation =
                createEAnnotation(AnnotationSources.SURJECTIVE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(surjectiveAnnotation);
            break;
          case INJECTIVE:
            final EAnnotation injectiveAnnotation = createEAnnotation(AnnotationSources.INJECTIVE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(injectiveAnnotation);
            break;
          case BIJECTIVE:
            final EAnnotation bijectiveAnnotation = createEAnnotation(AnnotationSources.BIJECTIVE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(bijectiveAnnotation);
            break;
          case COMPLETE:
            final EAnnotation completeAnnotation = createEAnnotation(AnnotationSources.COMPLETE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(completeAnnotation);
            break;
          case BIJECTION:
            final EAnnotation bijectionAnnotation = createEAnnotation(AnnotationSources.BIJECTION);
            // DEFAULT NULL
            eReference.getEAnnotations().add(bijectionAnnotation);
            break;
          case PREORDER:
            final EAnnotation preorderAnnotation = createEAnnotation(AnnotationSources.PREORDER);
            // DEFAULT NULL
            eReference.getEAnnotations().add(preorderAnnotation);
            break;
          case EQUIVALENCE:
            final EAnnotation equivalenceAnnotation =
                createEAnnotation(AnnotationSources.EQUIVALENCE);
            // DEFAULT NULL
            eReference.getEAnnotations().add(equivalenceAnnotation);
            break;
          case PARTIALORDER:
            final EAnnotation partialorderAnnotation =
                createEAnnotation(AnnotationSources.PARTIALORDER);
            // DEFAULT NULL
            eReference.getEAnnotations().add(partialorderAnnotation);
            break;
          case TOTALORDER:
            final EAnnotation totalorderAnnotation =
                createEAnnotation(AnnotationSources.TOTALORDER);
            // DEFAULT NULL
            eReference.getEAnnotations().add(totalorderAnnotation);
            break;
          case TRANSIENT:
            // DEFAULT FALSE
    ...
    if (modelFeatureAnnotation != null) {
      modelFeatureAnnotation.getContents().add(eReference);
      ((EClass) owner).getEStructuralFeatures().remove(eReference);
      return null;
    } else
      return eReference;
  }
  ...
  @Override
  public EAnnotation visitReason(ReasonContext ctx) {
    final String source = AnnotationSources.REASON;
    final EAnnotation eAnnotation = createEAnnotation(source);
